...
async def test_tool_roster():
    ...
    assert tools == {
        "list_teams", "list_agents", "spawn_agent", "dispatch_reconciler",
        "agent_status", "tail_agent", "exec_in_agent", "send_to_agent",
        "terminate_agent", "provision_gpu", "list_pods", "terminate_pod",
    }
...
async def test_implementer_cannot_spawn_or_manage():
    ...
    with pytest.raises(Exception, match="may not spawn"):
        await server.call_tool("spawn_agent", {
...
async def test_team_orch_confined_to_own_team():
    ...
    with pytest.raises(Exception, match="own team"):
        await server.call_tool("exec_in_agent",
                               {"sandbox": "gtz-demo-gemm-impl1", "command": "id"})
    with pytest.raises(Exception, match="own team"):
        await server.call_tool("spawn_agent", {
...
async def test_reconciler_readonly_and_no_control():
    ...
    with pytest.raises(Exception, match="own team"):
        await server.call_tool("send_to_agent",
                               {"sandbox": "gtz-demo-attn-impl1", "message": "hi"})
    with pytest.raises(Exception, match="may not spawn"):
        await server.call_tool("spawn_agent", {
